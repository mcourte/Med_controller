...
# Import des Modèles
from models.entities import Users
    ...
    def decorator(*args, **kwargs):
        try:
            ...
            if role_code in {'DOC'}:
                ...
                print(f"PermissionError: User role is {role_code}, required 'Doctor' or 'DOC'")
                raise PermissionError("Vous n'avez pas les autorisations nécessaires pour effectuer cette action")
        except PermissionError as e:
            print(e)
            raise
...
def is_authenticated(f):
    ...
    def decorator(cls, session, *args, **kwargs):
        ...
        try:
            ...
            user = session.query(Users).filter_by(username=decoded['username']).one_or_none()
            if user is None:
...
def decorator(f):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            if decoded_token.get('role') not in ['DOC', 'PAT']:
                raise PermissionError("Vous n'avez pas les autorisations nécessaires pour effectuer cette action")
